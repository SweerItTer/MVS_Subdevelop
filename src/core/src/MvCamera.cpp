...
int CMvCamera::EnumDevices(unsigned int nTLayerType, MV_CC_DEVICE_INFO_LIST* pstDevList,
						   const char *strManufacturerName, MV_SORT_METHOD enSortMethod)
{
	// m_devList = pstDevList;
	return MV_CC_EnumDevicesEx2(nTLayerType, pstDevList, strManufacturerName, enSortMethod);
}
...
// MV_CC_DEVICE_INFO* CMvCamera::GetDeviceInfoEx(int index)
// {
//     MV_CC_DEVICE_INFO* pDevInfo = nullptr; // 初始化pDevList为nullptr，避免未初始化的使用
//     pDevInfo = m_devList->pDeviceInfo[index];
//     return pDevInfo; // 返回pDevList，它是一个指向MV_CC_DEVICE_INFO_LIST的指针。
// }
